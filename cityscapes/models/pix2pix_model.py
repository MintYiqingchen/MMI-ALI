...
class Pix2PixModel(BaseModel):
    ...
    def backward_D(self):
        ...
        pred_fake_A = self.netD_A(fake_AB.detach())
        fake_BA = torch.cat((self.real_B, self.fake_A), 1)
        pred_fake_B = self.netD_B(fake_BA.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake_A, False) + self.criterionGAN(pred_fake_B, False)
        # Real
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        pred_real_A = self.netD_A(real_AB)
        real_BA = torch.cat((self.real_B, self.real_A), 1)
        pred_real_B = self.netD_B(real_BA)
        self.loss_D_real = self.criterionGAN(pred_real_A, True) + self.criterionGAN(pred_real_B, True)
    ...
    def backward_G(self):
        ...
        fake_BA = torch.cat((self.real_B, self.fake_A), 1)
        pred_fake_B = self.netD_B(fake_BA)
        self.loss_G_GAN = self.criterionGAN(pred_fake_A, True) + self.criterionGAN(pred_fake_B, True) 
        # Second, G(A) = B
        self.loss_G_L1 = (self.criterionL1(self.fake_B, self.real_B) + self.criterionL1(self.fake_A, self.real_A))* self.opt.lambda_L1
        # combine loss and calculate gradients
